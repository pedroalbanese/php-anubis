<?php
/**
 * Anubis GCM (Galois/Counter Mode) implementation in PHP.
 * Compatible with Go's crypto/cipher interface and edgetk.
 * Includes the Anubis cipher implementation.
 * 
...
class Anubis {
    ...
    private function keySetup($key) {
        ...
        for ($r = 0; $r <= $R; $r++) {
            ...
            $this->roundKeyEnc[$r] = [$K0, $K1, $K2, $K3];
    ...
    public function encryptBlock($block) {
        return $this->cryptBlock($block, $this->roundKeyEnc);
    }
    
    public function decryptBlock($block) {
        return $this->cryptBlock($block, $this->roundKeyDec);
    }
    
    private function cryptBlock($block, $roundKey) {
        $R = count($roundKey) - 1;
        $state = [];
...
class AnubisGCM {
    ...
    private function _ghash($data) {
        ...
        foreach ($blocks as $block) {
            ...
            $result = $this->_int128_xor($result, $block_int);
    ...
    private function _gmult($x, $y) {
        ...
        for ($i = 127; $i >= 0; $i--) {
            // Get i-th bit of x
            $bit = $this->_int128_bit_get($x, $i);
            
            if ($bit === '1') {
                $z = $this->_int128_xor($z, $v);
            }
            
            // Reduce v if LSB is set
            $lsb = $this->_int128_bit_get($v, 0);
            
            if ($lsb === '1') {
                $v = $this->_int128_shift_right($v);
                $v = $this->_int128_xor($v, '0xE1000000000000000000000000000000');
            } else {
                $v = $this->_int128_shift_right($v);
            }
        }
    ...
    // Helper functions for 128-bit integers using GMP
    private function _bytes_to_int128($bytes) {
        if (strlen($bytes) !== 16) {
            throw new Exception("Input must be 16 bytes");
        }
        
        // Convert to hex and then to GMP integer
        $hex = bin2hex($bytes);
    ...
    private function _int128_xor($a, $b) {
        return gmp_xor($a, $b);
    }
    
    private function _int128_bit_get($int, $position) {
        // Check if bit at position is set
        $mask = gmp_pow(2, $position);
        $result = gmp_and($int, $mask);
        return gmp_cmp($result, 0) > 0 ? '1' : '0';
    }
    
    private function _int128_shift_right($int) {
        return gmp_div($int, 2); // Right shift by 1
    }
    ...
    private function _compute_tag($ciphertext, $associated_data) {
        ...
        // Pack as 64-bit big-endian integers (J for big-endian, 64-bit)
        ...
        if (strlen($this->nonce) == 12) {
            ...
            $s = (16 - (strlen($this->nonce) % 16)) % 16;
            $nonce_padded = $this->nonce . str_repeat("\x00", $s);
            $nonce_padded .= "\x00\x00\x00\x00\x00\x00\x00\x00";
            $nonce_padded .= pack('J', strlen($this->nonce) * 8);
            $J0 = $this->_ghash($nonce_padded);
        }
    ...
    public function encrypt($plaintext, $associated_data = '') {
        if (strlen($this->nonce) == 12) {
            $icb = $this->nonce . "\x00\x00\x00\x01";
        } else {
            $s = (16 - (strlen($this->nonce) % 16)) % 16;
            $nonce_padded = $this->nonce . str_repeat("\x00", $s);
            $nonce_padded .= "\x00\x00\x00\x00\x00\x00\x00\x00";
            $nonce_padded .= pack('J', strlen($this->nonce) * 8);
            $icb = $this->_ghash($nonce_padded);
        }
...
class GCMAnubis {
    ...
    public function __construct($key) {
        $this->cipher = new Anubis($key);
        $this->key = $key;
    }
    
    public function nonceSize() {
        return 12;
    }
    ...
    public function seal($nonce, $plaintext, $associatedData) {
        $gcm = new AnubisGCM($this->cipher, $nonce);
        list($ciphertext, $tag) = $gcm->encrypt($plaintext, $associatedData);
        return $ciphertext . $tag;
    }
    
    public function open($nonce, $ciphertext, $associatedData) {
        ...
        $gcm = new AnubisGCM($this->cipher, $nonce);
        return $gcm->decrypt($ciphertextOnly, $tag, $associatedData);
    }
}
...
function newGCMWithNonceAndTagSize($cipher, $nonce, $tagSize) {
    return new AnubisGCM($cipher, $nonce, $tagSize);
}

function newGCMWithNonce($cipher, $nonce) {
...
function test_compatibility() {
    ...
    echo "Plain:    " . substr($plaintext, 0, 50) . "...\n";
    echo "AAD:      " . $associatedData . "\n";
    
    $cipher = new Anubis($key);
    $gcm = new AnubisGCM($cipher, $nonce);
    
    // Encrypt
    list($ciphertext, $tag) = $gcm->encrypt($plaintext, $associatedData);
    
    echo "\nCiphertext: " . bin2hex(substr($ciphertext, 0, 25)) . "...\n";
    echo "Tag:        " . bin2hex($tag) . "\n";
    echo "CT length:  " . strlen($ciphertext) . " bytes\n";
    echo "Tag length: " . strlen($tag) . " bytes\n";
    
    // Decrypt (should succeed)
    $decrypted = $gcm->decrypt($ciphertext, $tag, $associatedData);
    echo "\nDecrypt:    " . ($decrypted === $plaintext ? "SUCCESS" : "FAILED") . "\n";
    
    if ($decrypted !== null) {
        echo "Decrypted:  " . substr($decrypted, 0, 50) . "...\n";
    } else {
        echo "Decrypted:  NULL (authentication failed)\n";
    }
    
    // Test with wrong tag (should fail)
    $wrongTag = str_repeat("\xFF", 16);
    $decryptedWrong = $gcm->decrypt($ciphertext, $wrongTag, $associatedData);
    echo "Wrong tag:  " . ($decryptedWrong === null ? "SUCCESS (rejected)" : "FAILED") . "\n";
    
    // Test with wrong associated data (should fail)
    $wrongAd = "Wrong Associated Data";
    $decryptedWrongAd = $gcm->decrypt($ciphertext, $tag, $wrongAd);
...
function anubis_encrypt_cli($key_hex, $infile = null, $aad = null, $outfile = null, $tag_size = 16, $nonce_hex = null) {
    try {
        ...
        $key_bits = strlen($key) * 8;
        if (!in_array($key_bits, [128, 160, 192, 224, 256, 288, 320])) {
            throw new Exception("Anubis key must be 128, 160, 192, 224, 256, 288, or 320 bits, got $key_bits bits");
        }
        ...
        if (strlen($plaintext) == 0) {
            throw new Exception("No plaintext data to encrypt");
        }
        ...
        $gcm = new AnubisGCM($cipher, $nonce, $tag_size);
        
        // Convert AAD
        if ($aad === null) {
            $aad_bytes = '';
        } elseif (is_string($aad)) {
            $aad_bytes = $aad;
        } else {
            $aad_bytes = $aad;
        }
        ...
        list($ciphertext, $tag) = $gcm->encrypt($plaintext, $aad_bytes);
        
        // Output format: nonce + ciphertext + tag (like standard GCM)
        ...
        if ($outfile) {
            ...
            fprintf(STDERR, "Key size: %d bits\n", $key_bits);
            fprintf(STDERR, "Nonce: %s\n", bin2hex($gcm->getNonce()));
            fprintf(STDERR, "Nonce size: %d bytes\n", strlen($gcm->getNonce()));
            fprintf(STDERR, "Ciphertext length: %d bytes\n", strlen($ciphertext));
            fprintf(STDERR, "Tag (%d bytes): %s\n", $tag_size, bin2hex($tag));
            fprintf(STDERR, "Total output: %d bytes\n", strlen($output));
        } else {
            echo $output;
        }
...
function anubis_decrypt_cli($key_hex, $infile = null, $aad = null, $outfile = null, $tag_size = 16) {
    try {
        $key = hex2bin($key_hex);
        ...
        if ($infile) {
            ...
            // Read from stdin
            fprintf(STDERR, "Reading ciphertext from stdin...\n");
        ...
        if (strlen($data) < $min_size) {
            throw new Exception("Data too short, need at least $min_size bytes for nonce + tag");
        }
        
        // Parse input: nonce (12 bytes) + ciphertext + tag (tag_size bytes)
        $nonce = substr($data, 0, 12);
        $tag = substr($data, -$tag_size);  // Last tag_size bytes
        $ciphertext = substr($data, 12, -$tag_size);  // Everything between nonce and tag
...
function test_gcm() {
    ...
    echo "Key:          " . bin2hex($key) . "\n";
    echo "Nonce:        " . bin2hex($nonce) . "\n";
    echo "Plaintext:    " . substr($plaintext, 0, 50) . "...\n";
    echo "Associated:   " . $associatedData . "\n";
    ...
    // Decrypt (should succeed)
    $decrypted = $gcm->decrypt($ciphertext, $tag, $associatedData);
    echo "\nDecrypted:    " . substr($decrypted, 0, 50) . "...\n";
    echo "Success:      " . ($decrypted === $plaintext ? "true" : "false") . "\n";
    
    // Test with wrong tag (should fail)
    $wrongTag = '';
    for ($i = 0; $i < strlen($tag); $i++) {
        $wrongTag .= chr((ord($tag[$i]) + 1) & 0xFF);
    }
    $decryptedWrong = $gcm->decrypt($ciphertext, $wrongTag, $associatedData);
    echo "Wrong tag:    " . ($decryptedWrong === null ? "true (rejected)" : "false") . "\n";
    
    // Test with wrong associated data (should fail)
    $wrongAd = "Wrong Associated Data";
    $decryptedWrongAd = $gcm->decrypt($ciphertext, $tag, $wrongAd);
    echo "Wrong AD:     " . ($decryptedWrongAd === null ? "true (rejected)" : "false") . "\n";
    
    // Test empty plaintext
    echo "\n" . str_repeat("-", 60) . "\n";
    echo "Empty plaintext test:\n";
    
    $emptyPlaintext = "";
    list($ciphertextEmpty, $tagEmpty) = $gcm->encrypt($emptyPlaintext, $associatedData);
    $decryptedEmpty = $gcm->decrypt($ciphertextEmpty, $tagEmpty, $associatedData);
    
    echo "Empty encrypt: " . bin2hex($ciphertextEmpty) . "\n";
    echo "Empty tag:     " . bin2hex($tagEmpty) . "\n";
    echo "Empty decrypt: " . ($decryptedEmpty === $emptyPlaintext ? "true" : "false") . "\n";
    
    // Test without associated data
    echo "\n" . str_repeat("-", 60) . "\n";
    echo "No associated data test:\n";
    
    list($ciphertextNoAd, $tagNoAd) = $gcm->encrypt($plaintext, "");
    $decryptedNoAd = $gcm->decrypt($ciphertextNoAd, $tagNoAd, "");
    
    echo "No AD encrypt: " . substr($decryptedNoAd, 0, 50) . "...\n";
    echo "No AD success: " . ($decryptedNoAd === $plaintext ? "true" : "false") . "\n";
}
